...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN o.status IN ('completed', 'processing') THEN o.session_id END) as purchases,
    ...
    ROUND(100.0 * COUNT(DISTINCT CASE WHEN o.status IN ('completed', 'processing') THEN o.session_id END) / NULLIF(COUNT(DISTINCT ce.session_id), 0), 2) as cart_to_purchase_rate
FROM visits v
